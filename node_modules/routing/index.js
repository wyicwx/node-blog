...
function setRouting() {
	...
	app.all('/*',function(req,res) {
		var reqUrl = url.parse(req.url).path,params;
		//如果请求为静态文件不使用路由转向
		if(publicFile.test(reqUrl)) return;	
		reqUrl = path.normalize(reqUrl);		//格式化url路径
		if(reqUrl.hasString("?")||reqUrl.hasString("#")) {
			params = reqUrl.split("?")[1]||reqUrl.split("#")[1];
			reqUrl = reqUrl.split("?")[0]||reqUrl.split("#")[0];
			
			params.split("&").forEach(function(n) {
				req.params[n.split("=")[0]] = n.split("=")[1];
			})
		}
